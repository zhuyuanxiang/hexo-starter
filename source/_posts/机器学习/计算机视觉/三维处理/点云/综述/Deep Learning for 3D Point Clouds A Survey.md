...
原论文链接：[https://arxiv.org/abs/1912.12033](https://arxiv.org/abs/1912.12033)
...
- 逐点MLP网络通常用作其他类型的网络学习逐点特征的基本构建块。
...
#### _4.1.1_   基于Region Proposal区域建议的方法
...
在现有的Single Shot检测器的按比例缩小的特征图中，不可避免地会丢失点云的部分空间信息，受到这一观察结果的启发，He等人[160]提出了SA-SSD检测器，以利用细粒度结构信息来提高定位精度。具体地说，他们首先将点云转换为张量，然后将其输入主干网络以提取多级特征。此外，还采用了带有点级监督的辅助网络来指导特征学习点云的结构。 实验结果表明，SA-SSD在KITTI BEVdetection基准测试的汽车类中排名第一。_（排名是指本论文提交时间：2020年6月12日）_
...
- 得益于常规的数据表示方式，基于投影的方法和基于离散化的方法都可以利用其2D图像对应的成熟网络体系结构。然而，基于投影的方法的主要局限性在于3D-2D投影造成的信息丢失，而基于离散化的方法的主要瓶颈是分辨率的提高导致计算量和存储量的增加。为此，基于索引结构的稀疏卷积是一种可行的方法，值得进一步研究。
- 基于点的网络是最常用的研究方法。然而，点表示自然没有明确的邻域信息，大多数现有的基于点的方法求助于昂贵的邻域搜索机制（例如KNN[79]或ball query[54]）。这从本质上限制了这些方法的效率，最近提出的点-体素联合表示[256]将是一个值得进一步研究的方向。
- 在点云分割中，从不平衡数据中学习仍然是一个具有挑战性的问题。尽管[65]、[203]、[205]等几种方法都取得了显著的总体效果，但它们在少数群体中的表现仍然有限。例如，RandLA Net[206]在Semantic3D的reduced-8子集上实现了76.0%的整体IoU，但是在hardscape类上实现了41.1%的非常低的IoU。
- 大多数现有方法[5]、[54]、[79]，[205]，[207]在小点云上工作（例如，1m×1m，有4096个点）。在实际应用中，深度传感器获取的点云通常是巨大的、大规模的。因此，有必要进一步研究大规模点云的有效分割问题。
- 少数论文[178]、[179]、[199]已经开始从动态点云中学习时空信息。期望时空信息有助于提高后续任务的性能，如3D物体识别，分割和三维重建。
